...
## Why Jev
A fast, typed, repeated judgment over short text. One call returns calibrated probabilities across several questions in about 150 ms for about $0.00003. Output tokens are free, so more questions cost almost nothing. A generative model adds latency, cost and a parsing step for the same answer and gives no per-question probability to threshold on. Measured against `gpt-oss-120b/20b` on the same 50 updates: 2-3x faster, 1.6-3.6x cheaper, 11/13 vs 0-3/13 watermelons caught (untuned prompt; see README caveats).
